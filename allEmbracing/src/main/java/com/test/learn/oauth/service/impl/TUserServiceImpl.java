...
import com.test.learn.oauth.mapper.TUserMapper;
...
public class TUserServiceImpl extends ServiceImpl<TUserMapper, TUser> implements ITUserService {
    ...
    public int saveTUser(TUser tUser) {
        int result = 0;
        if (tUserMapper.exists(new LambdaQueryWrapper<TUser>()
                .eq(TUser::getUsername,tUser.getUsername())
                .eq(TUser::getPassword,tUser.getPassword())
                .eq(TUser::getFullname,tUser.getFullname())
                .eq(TUser::getMobile,tUser.getMobile())
            )){
            result = tUserMapper.update(tUser,new LambdaQueryWrapper<TUser>()
                    .eq(TUser::getUsername,tUser.getUsername())
                    .eq(TUser::getPassword,tUser.getPassword())
                    .eq(TUser::getFullname,tUser.getFullname())
                    .eq(TUser::getMobile,tUser.getMobile())
            ...
            result = tUserMapper.insert(tUser);
        }
        return result;
    }

    @Override
    public AjaxResult updateTUser(TUser tUser) {
        int result = 0;
        if (tUserMapper.exists(new LambdaQueryWrapper<TUser>()
                .eq(TUser::getUsername,tUser.getUsername())
                .eq(TUser::getPassword,tUser.getPassword())
            ...
            result = tUserMapper.updateById(tUser);
        }
        ...
        public String importExcel(MultipartFile file) {
            ...
            try {
                EasyExcel.read(file.getInputStream(), TUser.class, new ReadListener<TUser>() {
                /**
                 * 单次缓存的数据量
                 */
                 ...
                 *临时存储
                 */
                private List<TUser> cachedDataList = ListUtils.newArrayListWithExpectedSize(BATCH_COUNT);
                ...
                @Override
                public void invoke(TUser data, AnalysisContext context) {
                    // 非必空字段校验
                ...
                private void saveData() {
                    ...
                    AtomicInteger failureNum = new AtomicInteger();
                    StringBuilder failureMsg = new StringBuilder();
                    for (TUser tUser : cachedDataList){
                        try{
                            BeanValidators.validateWithException(validator, tUser);
        ...
        public void export(HttpServletResponse response, TUser tUser){
            final Integer[] total = {tUserMapper.selectCount(new LambdaQueryWrapper<>(tUser)).intValue()};
            if(total[0] == 0){
                try {
                    ...
                    EasyExcel.write(response.getOutputStream(), TUserDTO.class).sheet("数据").doWrite(new ArrayList<>());
            ...
            if (sheetNum[0] <= maxSheetNum[0]){
                writExcel(response, sheetNum[0], oneSheetWriteCount[0],lastSheetWriteCount,tUser,null,0);
            } else {
                // 分成多个文件 压缩响应
                String filePath = TUserServiceImpl.class.getResource("/").getPath()+
                ...
                for (int f = 0; f < fileNum; f++) {
                    ...
                    CompletableFuture.runAsync(() -> {
                        ...
                        writExcel(response, maxSheetNum[0], oneSheetWriteCount[0],lastSheetWriteCount2,tUser,fileName , finalF);
                    },threadPoolTaskExecutor).whenComplete((v,e) -> {
                    countDownLatch.countDown();
                    });
                }
    ...
    public void writExcel(HttpServletResponse response,int sheetNum,int oneSheetWriteCount,int lastSheetWriteCount,
        TUser tUser,String fileName,int f){
        try {
            if (StrUtil.isEmpty(fileName)){
                ...
                response.setHeader("Access-Control-Expose-Headers", "Content-Disposition");
                response.setHeader("luxsan", "luxsan");
                try (ExcelWriter excelWriter = EasyExcel.write(response.getOutputStream(), TUserDTO.class).build()) {
                    handleExcel(excelWriter, sheetNum, oneSheetWriteCount, lastSheetWriteCount, tUser, f);
                } catch (IOException ignored) {}
            } else {
                try (ExcelWriter excelWriter = EasyExcel.write(fileName, TUserDTO.class).build()) {
                    handleExcel(excelWriter, sheetNum, oneSheetWriteCount, lastSheetWriteCount, tUser, f);
                }
            }
        } catch (Exception e) {
    ...
    private void handleExcel(ExcelWriter excelWriter, int sheetNum, int oneSheetWriteCount, int lastSheetWriteCount, TUser tUser, int f) {
        for (int i = 0; i < sheetNum; i++) {
            ...
            for (int j = 0; j < (i != sheetNum - 1 || i == 0 ? oneSheetWriteCount : lastSheetWriteCount); j++) {
                ...
                CompletableFuture.runAsync(() -> {
                    ...
                    List<TUser> data = tUserMapper.selectList(new LambdaQueryWrapper<TUser>(tUser));
